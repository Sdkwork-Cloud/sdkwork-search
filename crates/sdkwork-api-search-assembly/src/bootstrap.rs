//! Generated API assembly bootstrap for sdkwork-search.
...
pub fn assemble_api_router() -> ApiAssembly {
    ...
    router = router.merge(sdkwork_routes_search_app_api::gateway_mount(state));
    router = router.merge(sdkwork_routes_search_backend_api::gateway_mount(state));
    ApiAssembly { router }
}
